...
export default function HomeFeedScreen() {
  ...
  const handleAddComment = () => {
    if (!selectedVideo || currentComment.trim() === "") return;
    const updatedVideos = [...videos];
    const index = updatedVideos.findIndex((v) => v.id === selectedVideo.id);
    updatedVideos[index].comments.push({
      id: Date.now(),
      text: currentComment,
    });
    setVideos(updatedVideos);
  ...
  const renderCommentItem = ({ item }) => (
    <View style={styles.commentRow}>
      <Image
        source={{
          uri: "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/SubaruOutbackOnStreetsOfJapan.mp4",
        }}
        style={styles.avatar}
      />
      <Text style={styles.commentText}>{item.text}</Text>
      ...
      <InteractionContainer>
        ...
        <TouchableOpacity onPress={() => toggleBookmark(item.id)}>
          <Ionicons
            ...
            size={32}
            color={bookmarkedIds.includes(item.id) ? "#1E90FF" : "white"}
          />
        </TouchableOpacity>
      </InteractionContainer>
    </VideoContainer>
